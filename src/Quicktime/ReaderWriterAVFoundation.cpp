    ...
    public:
        ...
        virtual ReadResult readImage(const std::string& file, const osgDB::ReaderWriter::Options* options) const
        {
            ...
            if (!options
                ...
                && disable_core_video))
            {
                static osg::ref_ptr<cefix::VideoFrameDispatcher> video_frame_dispatcher(NULL);
                if (!video_frame_dispatcher) {
                    std::string num_threads_str = options ? options->getPluginStringData("numFrameDispatchThreads") : "0";
                    video_frame_dispatcher = new cefix::VideoFrameDispatcher(atoi(num_threads_str.c_str()));
                }

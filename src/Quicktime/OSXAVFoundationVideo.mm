...
void OSXAVFoundationVideo::pause()
{
    ...
    PlatformUtils::NSAutoreleasePoolHelper helper;
...
void OSXAVFoundationVideo::open(const std::string& filename)
{
    ...
    {
        url = [NSURL URLWithString: PlatformUtils::toNSString(filename)];
    }
    else
    {
        url = [NSURL fileURLWithPath: PlatformUtils::toNSString(filename)];
    }
...
void OSXAVFoundationVideo::decodeFrame()
{
    ...
    if (!is_valid)
    {
        _waitForFrame = false;
        pause();
        OSG_WARN << "OSXAVFoundationVideo: " << PlatformUtils::toString([_data->avplayer.error localizedFailureReason]) << std::endl;
    }

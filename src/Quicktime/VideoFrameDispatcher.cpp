...
#include <cefix/VideoFrameDispatcher.h>
#include <iostream>
#include <osg/Timer>
...
void VideoFrameDispatchQueue::addItem(VideoImageStream *stream)
{
    if (_finished) return;
...
void VideoFrameDispatchQueue::removeItem(VideoImageStream* stream)
{
    stream->setDispatchQueue(NULL);
    OpenThreads::ScopedLock<OpenThreads::Mutex> lock(_mutex);
    _queue.erase(stream);

...
OSXQTKitVideo::OSXQTKitVideo()
    : cefix::VideoImageStream()
    , _rate(0.0)
    , _coreVideoAdapter(NULL)
{
    PlatformUtils::PlatformUtils::NSAutoreleasePoolHelper autorelease_pool_helper;
...
{
    bool valid = true;
    PlatformUtils::NSAutoreleasePoolHelper autorelease_pool_helper;
    ...
    if (osgDB::containsServerAddress(file_name))
        [movieAttributes setObject:[NSURL URLWithString: PlatformUtils::toNSString(file_name)] forKey: QTMovieURLAttribute];
    else
        [movieAttributes setObject:[NSURL fileURLWithPath: PlatformUtils::toNSString(file_name)] forKey: QTMovieURLAttribute];
...
void OSXQTKitVideo::setTimeMultiplier(double r)
{
    ...
    PlatformUtils::NSAutoreleasePoolHelper pool;
    _rate = r;
